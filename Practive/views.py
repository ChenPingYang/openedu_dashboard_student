    ...
    if request.method == "GET":
        ...
        if islogin is True:
            ...
            if cookiegetter.isTeacher(userID):
                    ...
                    for rs in result:
                        for i in range(len(teachersCourse)):
                            if rs.course_id == teachersCourse[i]:
                                ...
                                data.append('{:,.2f}'.format(rs.作答過半人數))
                                data.append(rs.練習題作答率)
                                data.append(rs.練習題答對率)
                ...
                to_render['IsLogin'] = 1
                print('teacher')
                return render(request, '4_PractiveQuestionsData.html', to_render)
